const XMLHttpRequest = require('xmlhttprequest').XMLHttpRequest; // carga lo requerido
const API = 'https://api.escuelajs.co/api/v1'; // esta será la url que queremos consultar

function fetchData(urlApi, callback) { // función principal que obtendrá los datos como un objeto
    let xhttp = new XMLHttpRequest(); // inicializar objeto de tipo XMLHttpRequest
    ...
    xhttp.onreadystatechange = function(event) { //en este metodo Almacena el nombre de la función que se ejecutará cuando el objeto XMLHttpRequest cambie de estado
        if (xhttp.readyState === 4) { // estados posibles: 0 -> no inicializado; 1 -> loading; 2 -> ejecutado; 3 -> interactuando; 4 -> completado
            if (xhttp.status === 200) { // 200 es solicitud correcta
                ...
                const error = new Error('Error' + urlApi);  //si la respuesta de la API no es exitosa se captura el error
...
fetchData(`${API}/products`, function (error1, data1) {
    ...
    fetchData(`${API}/products/${data1[0].id}`, function(error2, data2){ //se invoca nuevamente la función fetchData con el fin de acceder a un objeto puntual del arreglo data1, se envia como parámetros la url de la API apuntando al atributo del primer objeto de arreglo data1 y nuevamente una función anónima.
        ...
        //igual que las anteriores se envia una funcion anonima con 2 argumentos, un objeto Error y un objeto de datos
            ...
            console.log(data3.name); //Se imprime el nombre de la categoria a la que pertenece el objeto que se consultó en la segunda invocación del método.
        });
    });
});